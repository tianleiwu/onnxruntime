...
- **`UpsampleBase`** — depends on `InputDefs()` and `OpKernelInfo::GetAllocator()`. Plugin wraps the underlying implementation differently (see excluded ops).
...
| `tensor/space_depth_ops.cc` | Inherits `SpaceDepthBase` (CPU provider) | Inline `SpaceDepthBase::SpaceDepthCompute` into header with `SHARED_PROVIDER` guard, or inline in adapter. |
| `tensor/upsample.cc` | `UpsampleBase` uses `InputDefs()` and `OpKernelInfo::GetAllocator()` | Extend adapter `OpKernelInfo` with `GetAllocator()` and `InputDefs()` support. |
...
| `object_detection/*` | `NonMaxSuppressionBase`, `RoiAlignBase` from CPU provider | Inline base class methods or add CPU shims. |
...
| `contrib_ops/cuda/bert/attention.cc` | `GetComputeStream()` returns real `Stream*` which is needed | Same `Stream*` adapter extension. |
...
| `contrib_ops/cuda/bert/longformer_attention.cc` | Same | Same. |
...
| `contrib_ops/cuda/tensor/crop.cc` | Framework deps | Audit specific deps. |
...
2. **CPU base class inheritance** (~5 ops) — Some ops inherit from CPU base classes not linked into the plugin. Most have been refactored; remaining ones (`SpaceDepthBase`, `UpsampleBase`, object detection bases) need similar treatment.
