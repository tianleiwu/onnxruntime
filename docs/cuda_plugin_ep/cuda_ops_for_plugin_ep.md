...
| `tensor/space_depth_ops.cc` | Inherits from `SpaceDepthBase` (CPU provider, `core/providers/cpu/tensor/space_depth_ops.h`). | Add `#ifdef BUILD_CUDA_EP_AS_PLUGIN` self-contained class in the header that inlines `SpaceDepthBase` logic (blocksize attribute parsing, shape validation). Moderate effort. |
| `tensor/upsample.cc` | Inherits from `UpsampleBase` (CPU provider). `UpsampleBase` uses `InputDefs()` and complex attribute/input parsing in its constructor. | Add `#ifdef BUILD_CUDA_EP_AS_PLUGIN` path or inline `UpsampleBase` logic. Significant effort due to multi-version attribute handling (scales vs. sizes). |
...
| `object_detection/*` (NonMaxSuppression, RoiAlign) | `NonMaxSuppression` inherits from `NonMaxSuppressionBase`; `RoiAlign` inherits from `RoiAlignBase`. Both CPU base classes. `NonMaxSuppression` also uses CPU helper `PrepareCompute`. | Add `#ifdef BUILD_CUDA_EP_AS_PLUGIN` self-contained classes. NMS is moderately complex (box sorting, IOU computation partially on CPU). RoiAlign is simpler. |
...
| **bert/attention.cc** | Calls `GetScratchBuffer` with `context->GetComputeStream()` (works via adapter). Main issue: calls `QkvToContext` passing `context->GetComputeStream()` (`Stream*`), and uses `IAllocator::MakeUniquePtr` with stream. | Change `QkvToContext` signature or wrap stream. Moderate-high effort due to attention pipeline complexity. |
...
| **bert/embed_layer_norm.cc** | No direct `GetComputeStream()` usage found; may have transitive header dependency. | Verify compilation with exclusion removed — may work with adapter. Low effort to try. |
| **bert/fast_gelu.cc** | Transitively includes `cudnn_common.h` or other framework headers. | Verify compilation — may already work with adapter headers. Low effort to try. |
...
| **bert/longformer_attention.cc** | Uses `GetScratchBuffer` with `GetComputeStream()`, workspace allocation. | Replace stream access pattern. Moderate effort. |
...
| **tensor/crop.cc** | No `GetComputeStream()` usage found in the source. May have header-level deps. | Verify compilation — may already work. **Low effort to try.** |
...
|------|-------------|-------|
| `tensor/reshape.cc` | `GetComputeStream`/`CopyTensor` fixed via adapter `CudaKernel` base class. | 5A |
...
| `math/integer_gemm.cc` | `dynamic_cast<CudaStream*>` replaced with `GetCublasHandle(cudaStream_t)`. | 5 |
...
already supports. They should be tried first:

- `contrib/bert/embed_layer_norm.cc`
- `contrib/bert/fast_gelu.cc`
- `contrib/bert/relative_attn_bias.cc`
- `contrib/bert/remove_padding.cc`
- `contrib/tensor/crop.cc`
...
- `tensor/space_depth_ops.cc` — inline `SpaceDepthBase`
...
- `tensor/upsample.cc` + `tensor/resize.cc` — large `UpsampleBase` inline
...
- `object_detection/*` — CPU base class inline
