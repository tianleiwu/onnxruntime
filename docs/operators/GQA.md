...
## 9. Testing
### Build and Test
If you change C++ code, you can need build and install a new python wheel before testing:
Release build: `bash gqa_release.sh --build --install`
Debug build: `bash gqa.sh --build --install`
Debug build is recommended for developing and debugging.
Release build is recommended for benchmarking and profiling.
For gqa.sh, it enables `onnxruntime_DEBUG_NODE_INPUTS_OUTPUTS=ON`,`onnxruntime_DUMP_TENSOR=ON` and `onnxruntime_QUICK_BUILD=ON` automatically. Quick build only build head dimension=128 for flash attention, so it could speed up the build process.

For gqa_release.sh, you can manually enable `onnxruntime_QUICK_BUILD=ON` by `--quick_build`, and `onnxruntime_DUMP_TENSOR=ON` by `--dump`.

If you need run test together with build, you can append `--test` to the command line, like
./gqa.sh --build --install --test

You can also run test only if the wheel is already installed, like `./gqa.sh --test`.

If you need run a specified test, you can run like `./gqa.sh --test_case <test_case_name>`.
For debug build, you can append `--dump` to the command line, like
./gqa.sh --dump --test_case <test_case_name>
This will dump the input and output of the test case, and also dump tensors by DUMP_TENSOR in C++ code.
