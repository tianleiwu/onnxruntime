# INT4 Group-wise MoE Quantization Format

This document describes the tensor formats for **float16 input with INT4 group-wise quantization** in the QMoE operator.
The format is same as the corresponding one used in TensorRT-LLM Mixture of Experts (MoE) plugin.
...
> [!NOTE]
> W4A8 (FP8 activations with INT4 weights) is **not currently implemented** in ONNX Runtime.
> The scale interleaving described below is from TRT-LLM reference, kept for future work (P3 in the plan).
...
- Weight preprocessing: [`cutlass_preprocessors.cpp`](https://raw.githubusercontent.com/NVIDIA/TensorRT-LLM/main/cpp/tensorrt_llm/kernels/cutlass_kernels/cutlass_preprocessors.cpp)
- INT4 unpacker: `torch.ops.trtllm.unpack_int4_packed_tensor_to_int8`
- QuantParams struct: [`moe_kernels.h`](https://raw.githubusercontent.com/NVIDIA/TensorRT-LLM/main/cpp/tensorrt_llm/kernels/cutlass_kernels/include/moe_kernels.h#L242-L419)
- MoE Plugin: [`mixtureOfExpertsPlugin.h`](https://raw.githubusercontent.com/NVIDIA/TensorRT-LLM/main/cpp/tensorrt_llm/plugins/mixtureOfExperts/mixtureOfExpertsPlugin.h)
- Test reference: [`test_moe_weight_only_quant_matmul.py`](https://raw.githubusercontent.com/NVIDIA/TensorRT-LLM/main/tests/unittest/trt/quantization/test_moe_weight_only_quant_matmul.py)
- Groupwise test: [`test_weight_only_groupwise_quant_matmul.py`](https://raw.githubusercontent.com/NVIDIA/TensorRT-LLM/main/tests/unittest/trt/quantization/test_weight_only_groupwise_quant_matmul.py)

---

## ORT-Specific Notes

- **SM120 (RTX 5090)**: INT4 weights on SM120 use the **Ampere GemmGrouped** fallback path (same weight packing as SM80/86/89). `min_dim=128` applies.
- **SM90 (Hopper)**: INT4 weights on SM90 also use the Ampere GemmGrouped fallback (TMA WS only activates for same-type fp16×fp16 or bf16×bf16).
- **Weight packing compatibility**: SM80/86/89/100/120 share the same packed format (Group A). SM90 uses a distinct format (Group B). See `moe_qmoe_overview.md` Section 6.3.
- **W4A8 (FP8 activations)**: Not currently implemented in ONNX Runtime. The Hopper W4A8 scale interleaving described above is TRT-LLM reference for future work.
