...
#### Already exists (disabled behind `#if 0`)

| Component | File |
|-----------|------|
| `MoeGemmRunner<__nv_fp8_e4m3, __nv_fp4_e2m1, half>` | `moe_gemm/moe_gemm_kernels_fp8_fp4.cu` |
| Block-scaled dispatch for FP4 | `moe_gemm/moe_gemm_template_dispatch_tma_ws.h` (`#ifdef ENABLE_FP4` sections) |
| Block-scaled traits for FP4 | `moe_gemm/moe_tma_warp_specialized_traits.h` |

#### Changes needed

1. **Schema** (`contrib_defs.cc`): Add inputs 21–22 for activation scales.

2. **Kernel instantiation** (`moe_gemm_kernels_fp8_fp4.cu`): Remove `#if 0` guard, enable the template.
