# INT8 Group-wise QMoE Format

This document describes the tensor formats for **float16 input with INT8 group-wise quantization** in the QMoE operator.
...
- Test reference: [`test_weight_only_groupwise_quant_matmul.py`](https://raw.githubusercontent.com/NVIDIA/TensorRT-LLM/main/tests/unittest/trt/quantization/test_weight_only_groupwise_quant_matmul.py)

---

## ORT-Specific Notes

- **SM120 (RTX 5090)**: INT8 weights on SM120 use the **Ampere GemmGrouped** fallback path (same weight packing as SM80/86/89). `min_dim=128` applies.
- **SM90 (Hopper)**: INT8 weights on SM90 also use the Ampere GemmGrouped fallback (TMA WS only activates for same-type fp16×fp16 or bf16×bf16).
...
- **W8A8 (FP8 activations)**: Not currently implemented in ONNX Runtime. Planned as future work (P3 in the plan) targeting SM89 (RTX 4090).
