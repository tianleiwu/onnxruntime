...
single bucket is profiled lazily on first use and merged into the cache. Set
`ORT_FPA_INTB_PROFILE_M` (comma-separated) to override the bucket set; its
maximum also sets the initial profile range.
...
written and behavior is unchanged. The cache is process-global (shared across all
`MatMulNBits` nodes and sessions); the first constructed kernel fixes the
location.
