...
# Licensed under the MIT License.
  ...
  function(config_cuda_provider_shared_module target)
    ...
    set(ort_cuda_architectures_for_tma ${CMAKE_CUDA_ARCHITECTURES})
    if(DEFINED CMAKE_CUDA_ARCHITECTURES_ORIG)
      set(ort_cuda_architectures_for_tma ${CMAKE_CUDA_ARCHITECTURES_ORIG})
    endif()

    set(ort_cuda_has_hopper_tma FALSE)
    set(ort_cuda_has_blackwell_tma FALSE)
    set(ort_cuda_has_blackwell_sm120_tma FALSE)
    foreach(cuda_arch IN LISTS ort_cuda_architectures_for_tma)
      if(cuda_arch MATCHES "^90a?(-real|-virtual)?$")
        set(ort_cuda_has_hopper_tma TRUE)
      endif()
      if(cuda_arch MATCHES "^100a?(-real|-virtual)?$")
        set(ort_cuda_has_blackwell_tma TRUE)
      endif()
      if(cuda_arch MATCHES "^(120a?|121a?)(-real|-virtual)?$")
        set(ort_cuda_has_blackwell_sm120_tma TRUE)
      endif()
    endforeach()

    if(ort_cuda_has_hopper_tma OR ort_cuda_has_blackwell_tma OR ort_cuda_has_blackwell_sm120_tma)
      target_compile_options(${target} PRIVATE $<$<COMPILE_LANGUAGE:CUDA>:-Xptxas=-w>)
    endif()
    if(ort_cuda_has_hopper_tma)
