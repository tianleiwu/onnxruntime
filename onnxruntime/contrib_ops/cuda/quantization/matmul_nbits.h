    ...
    if constexpr (std::is_same<T, MLFloat16>::value || std::is_same<T, BFloat16>::value) {
      ...
      if (prepacked) {
        ...
        ORT_ENFORCE(has_fpA_intB_gemm_,
                    ...
                    "INT2 (block_size=64), INT4 or INT8 (block_size=32) weights, scale-only quantization "
                    "without zero points, bias, or g_idx, the SM80 weight layout (weight_prepacked=1), "
                    "and compute capability 7.5 or later. Got bits=",
