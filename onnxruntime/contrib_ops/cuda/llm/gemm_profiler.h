...
class GemmPluginProfiler {
 public:
  ...
  // lazily (single bucket), inserts it into the in-process map, and persists it to the disk
  // cache. This briefly blocks the caller but guarantees a tuned tactic for any runtime M,
  // which is what makes the reduced first-time M sweep safe.
...
std::optional<Config> GemmPluginProfiler<Config, RunnerPtr, GemmIdType, GemmIdHashType>::getBestConfigOrProfile(
  ...
  // Persist the newly profiled bucket to the disk cache (if configured).
  storePersistentCache(gemmId, *mProfileMap, mHasWeightOnlyCudaKernel);
