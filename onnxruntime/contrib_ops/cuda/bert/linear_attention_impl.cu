...
__global__ void LinearAttentionRecurrentKernel(
  ...
  // Global state pointers for this (batch, head): [d_k, d_v]. They may alias exactly.
  const int64_t state_offset = ((int64_t)b * kv_num_heads + h_kv) * d_k * d_v;
...
__global__ void LinearAttentionRecurrentKernelFixedShape(
  ...
  const int64_t state_offset = ((int64_t)b * kv_num_heads + h_kv) * DK * DV;
  const T* S_past = past_state + state_offset;
  T* S_present = present_state + state_offset;
...
__global__ void LinearAttentionDecodeKernel(
  ...
  const int64_t state_offset = ((int64_t)b * kv_num_heads + h_kv) * DK * d_v + col;
