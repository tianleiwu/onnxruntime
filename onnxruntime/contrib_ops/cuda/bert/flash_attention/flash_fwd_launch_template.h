...
// Replace C10 macros with CUDA-native equivalents
#define C10_CUDA_CHECK(err) do { \
    cudaError_t _check_err_ = (err); \
    if (_check_err_ != cudaSuccess) { \
        fprintf(stderr, "CUDA error: %s\n", cudaGetErrorString(_check_err_)); \
    } \
} while(0)
#define C10_CUDA_KERNEL_LAUNCH_CHECK() do { \
    cudaError_t _launch_err_ = cudaGetLastError(); \
    if (_launch_err_ != cudaSuccess) { \
        fprintf(stderr, "CUDA kernel launch error: %s\n", cudaGetErrorString(_launch_err_)); \
    } \
} while(0)

#include "contrib_ops/cuda/bert/flash_attention/static_switch.h"
#include "contrib_ops/cuda/bert/flash_attention/hardware_info.h"
                ...
                BOOL_SWITCH(return_softmax, ReturnSoftmaxConst, [&] {
                    ALIBI_SWITCH(params.alibi_slopes_ptr != nullptr, Has_alibi, [&] {
                        SOFTCAP_SWITCH(params.softcap > 0.0, Is_softcap, [&] {
                            ...
                            auto kernel = &flash_fwd_kernel<Kernel_traits, Is_causal, Is_local && !Is_causal, Has_alibi, IsEvenMNConst && IsEvenKConst && !Is_local && !Has_alibi && !ReturnSoftmaxConst && Kernel_traits::kHeadDim <= 128, IsEvenKConst && !ReturnSoftmaxConst && !Has_alibi, Is_softcap, false>;
