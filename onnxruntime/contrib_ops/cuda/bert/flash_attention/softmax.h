...
struct Softmax {
  ...
  __forceinline__ __device__ void softmax_rescale_o(Tensor0& acc_s, Tensor1& acc_o, float softmax_scale_log2) {
    ...
    if constexpr (Is_first) {
      flash::reduce_max<kNThreadsPerRow, /*zero_init=*/true>(scores, row_max);
      flash::scale_apply_exp2(scores, row_max, softmax_scale_log2);
      flash::reduce_sum</*zero_init=*/true>(scores, row_sum);
    } else {
      TensorT row_max_prev = make_fragment_like(row_max);
...
#pragma unroll
      for (int mi = 0; mi < size<0>(row_max); ++mi) {
        ...
        float scores_scale = exp2f((row_max_prev(mi) - scores_max_cur) * softmax_scale_log2);
