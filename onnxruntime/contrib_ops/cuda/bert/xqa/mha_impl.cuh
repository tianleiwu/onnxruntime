...
#pragma unroll
    ...
    if constexpr (needMergeGlobal) {
      ...
      for (uint32_t i = 1; i < nbXTilesPerXIter; i++) {  // i = 0 is for self and we can skip
        ...
        assert(all(smem.warpRowMax[warpIdx.y][otherWarpIdx].template loadToReg<false>(warp) == smem.warpRowMax[warpIdx.y][otherWarpIdx - warpIdxInGrp].template loadToReg<false>(warp)));
...
__device__ __host__ inline size_t GetScratchSize(uint32_t nbSeq, uint32_t nbSubSeqPerSeq) {
  ...
  // 1. rowMax
  offset = roundUp<size_t>(offset, alignof(SMemWarpRowMax));
  ...
  offset = roundUp<size_t>(offset, alignof(VecT));
