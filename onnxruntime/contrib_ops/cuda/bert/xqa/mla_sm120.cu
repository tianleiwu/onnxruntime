...
struct KVTilePartLoader {
  ...
  static inline constexpr uint32_t nbPagesPerTile = tokensPerTile >= tokensPerPage ? exactDiv(tokensPerTile, tokensPerPage) : 1;
#endif
...
__device__ inline KVTilePartLoader::KVTilePartLoader(
    ...
    : cacheList{cacheList}, idxReq{idxReq}, tensorMap{tensorMap}
#if USE_PAGED_KV_CACHE
      ,
      nbPages{nbPages}
...
#if USE_PAGED_KV_CACHE
  uint32_t const idxPageBeg = tokensPerTile >= tokensPerPage ? nbPagesPerTile * idxTile : idxTile / exactDiv(tokensPerPage, tokensPerTile);
...
struct SharedMemA {
  ...
  __device__ inline void invalidateBarriers(uint32_t thrdIdx) {
    ...
    constexpr uint32_t nbBarsRef = exactDiv(offsetof(SharedMemA, qkScaleLog2e) - offsetof(SharedMemA, rowMaxLog2eBar), 8);
...
struct SharedMemB {
  ...
  using VBuffer = Vec<Array2D<LdGrain, tokensPerTile, exactDiv(partElemsV, grainElems)>, exactDiv(gemm1V, partElemsV)>;
...
struct Producer {
  ...
  __device__ inline Producer(KernelArgs const& args, SharedMemA& smem, uint32_t const maxNbSubSeq,
      ...
      : args(args), smem(smem), maxNbSubSeq(maxNbSubSeq), idxReq(idxReq), idxInputTokenGlobal(idxInputTokenGlobal), seqLen(seqLen), nbSubSeq(nbSubSeq), idxSubSeq(idxSubSeq), ctaRank(ctaRank), warpRank(warpRank), warpIdx(warpIdx) {
#ifndef NDEBUG
    if (threadIdx.x == 0) {
...
#pragma unroll 1
    for (uint32_t grpIter = 0; true; grpIter++) {
      ...
      auto shouldTestWait = [](uint32_t idxInstK, uint32_t idxAtomBx2) { return idxInstK == partNbInstK - 1 && idxAtomBx2 == tileNbAtomBx2 - 2; };
          ...
          for (uint32_t idxAtomBx2 = 0; idxAtomBx2 < tileNbAtomBx2; idxAtomBx2++) {
            if (idxAtomBx2 == 2 && prefetch) {
              if (idxPartPrefetch < SharedMemA::regQParts) {
                ...
                regQBuf[idxInstKPrefetch] = loadRegQCol(smem.q[idxPartPrefetch - SharedMemA::regQParts], idxInstKPrefetch);
              }
            }
...
__device__ inline Producer::WarpAcc Producer::scaleAndSoftmax(
  ...
  CgaBarrier& nextRowMaxLog2eBar = smemNext.rowMaxLog2eBar[(cgaIter + 1) % (nbMathGrps * nbProducerCtasPerCga) / nbMathGrps];
...
#pragma unroll
      for (uint32_t idxAtomBx2 = 0; idxAtomBx2 < warpTileNbAtomBx2; idxAtomBx2++) {
        auto const data = ldmatrix_16x16_trans<2>(&vBuf.template at<true>(qmmaShape.k * idxInstK + rB, idxAtomBx2 + cB));
...
#pragma unroll
  ...
  WarpOutSwizzleBuf& swizzleBuf = reinterpret_cast<Vec<WarpOutSwizzleBuf, nbWarpOutSwizzleBuf>&>(smem.xv[idxXVBufLast])[warpRank];
  ...
  if (isMultiBlockMode && tileIdx.x == 0) {
    ThrdRegRowMax const accRowMaxLog2e = loadShmRowMax<warpTile.y>(smem.accRowMaxLog2e[tileIdx.x], tileBase.y, lane);
...
#pragma unroll 1
    ...
    auto loadBuf = [&](RowWise& rowMaxLog2e, RowWise& rowSum, RegChunk& regChunk, PartialResult::Chunk const& chunk) {
      auto loadRowWise = [&](Vec<float, PartialResult::nbRowsPerChunk> const& src) { return reinterpret_cast<RowWise const&>(src[tileRowBase]); };
    ...
    PartialResult* __restrict__ const partialResults = nullptr)         // [totalNbInputTokens][maxNbSubSeq]
{
  ...
  uint32_t const inputSeqLen = (allowMultipleInputTokens ? exactDiv(gridDim.x, cgaSize) : checkedVal(1U, exactDiv(gridDim.x, cgaSize)));
  uint32_t const reqIdxInputToken = (allowMultipleInputTokens ? blockIdx.x / cgaSize : checkedVal(0U, blockIdx.x / cgaSize));
...
void launchMLA(cudaDeviceProp const& prop,
  ...
  auto const tensorMapQ = makeTensorMapForQ(q, dtype, validElemsPerHead, headGrpSize * inputSeqLen * batchSize, partElemsK);
#if PAGED_KV_CACHE_LAYOUT == 1
