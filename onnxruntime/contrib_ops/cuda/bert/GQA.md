...
### 4.4 Mixed Position ID Strategy


### 4.4 Mixed Position ID Strategy

The implementation uses a **mixed strategy** for position IDs to balance performance and correctness:

1.  **Query (Q) Rotation**: Uses **explicit position IDs** generated by `LaunchSeqlensToPosIds`.
    -   Necessary for Flash Attention path where `LaunchRotaryEmbeddingKernel` requires a valid input buffer.
    -   Ensures correct handling of all prompt/decode scenarios.

2.  **Key (K) Rotation**: Uses **implicit position IDs** calculated internally by fused kernels.
    -   `ConcatKVInPlaceFused` and `ConcatNewToPastKV` accept `nullptr` for `position_ids`.
    -   They compute `pos_id = past_seq_len + s` on-device.
    -   Reduces global memory reads and kernel argument overhead.

```cpp
// For prompts: position_ids[b,s] = s
// For tokens: position_ids[b,s] = past_seqlen + s
// Only generated if explicit Q rotation is needed AND user didn't provide position_ids!
if (position_ids == nullptr) {
    ORT_RETURN_IF_ERROR(LaunchSeqlensToPosIds(parameters, seqlens_k, position_ids_buffer, ...));
}
```
...
|------|------------------|-----------------|---------------|
| Unpacked + Share Buffer + RoPE | 4 | 3 | ~2-3 μs |
