...
Status LaunchTransCtx(cudaStream_t stream,
  ...
  if (0 == (head_size & 1)) {
    ...
    if (head_size * num_heads <= max_threads_per_block) {
      const dim3 block(head_size, num_heads, 1);
      TransposeCtx<__nv_bfloat16><<<grid, block, 0, stream>>>(head_size, reversed_bs, input, output);
    } else {
      const dim3 block(max_threads_per_block / num_heads, num_heads, 1);
      TransposeCtxLarge<__nv_bfloat16><<<grid, block, 0, stream>>>(head_size, reversed_bs, input, output);
    }
  }
...
Status Transpose_BSNH_to_BNSH(const int batch_size, const int sequence_length, const int num_heads, const int head_size,
                              const __nv_bfloat16* input, __nv_bfloat16* output, cudaStream_t stream, const int max_threads_per_block) {
  return LaunchTransQkv(stream, 1, sequence_length, batch_size, head_size, num_heads,
                        max_threads_per_block, false, input, output);
}
