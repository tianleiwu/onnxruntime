...
Status MatMulBlockScaledFp8::ComputeInternal(OpKernelContext* context) const {
  ...
  // Restricted to the FP8 A -> BF16 Y case with full K scale blocks and K/N aligned to 16
  // (fp8 128-bit access). Incomplete K blocks are handled by the scalar fallback below.
  if (!fp16_io && output->IsDataType<BFloat16>() && block_size_ == 128 &&
      (k % block_size_ == 0) && (n % 16 == 0)) {
