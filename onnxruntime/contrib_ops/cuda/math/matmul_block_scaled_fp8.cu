...
#pragma unroll
            for (int i = 0; i < 4; ++i) {
              const float2 v = Fp8GemvToFloat2(av[i]);
              a_f[r][2 * i] = v.x;
              a_f[r][2 * i + 1] = v.y;
            }
          }
...
__global__ void MatMulBlockScaledFp8MmaGemvKernel(AType* __restrict__ output,
  ...
  const bool a_ok = g < m;
  const size_t a_off = static_cast<size_t>(g) * k + (t << 4);

  float acc[4] = {0.f, 0.f, 0.f, 0.f};   // scaled, summed over K blocks
...
#pragma unroll
  ...
  if constexpr (KSplit > 1) {
    __shared__ float red[KSplit * 32 * 4];
    float* slot = red + (warp * 32 + lane) * 4;
#pragma unroll
    for (int i = 0; i < 4; ++i) {
      slot[i] = acc[i];
    }
...
#pragma unroll
    ...
    for (int ws = 0; ws < KSplit; ++ws) {
      const float* p = red + (ws * 32 + lane) * 4;
#pragma unroll
      for (int i = 0; i < 4; ++i) {
        acc[i] += p[i];
      }
  ...
  // Lane (g, t) owns y[2t, col_lo], y[2t + 1, col_lo], y[2t, col_hi], y[2t + 1, col_hi].
  const int row = t << 1;
  const float bias_lo = (bias != nullptr && lo_ok) ? ToFloat<AType>(bias[col_lo]) : 0.f;
  const float bias_hi = (bias != nullptr && hi_ok) ? ToFloat<AType>(bias[col_hi]) : 0.f;
  if (row < m) {
    if (lo_ok) {
      output[static_cast<size_t>(row) * n + col_lo] = FromFloat<AType>(acc[0] + bias_lo);
    }
    if (hi_ok) {
      output[static_cast<size_t>(row) * n + col_hi] = FromFloat<AType>(acc[2] + bias_hi);
    }
  }
  if (row + 1 < m) {
    if (lo_ok) {
      output[static_cast<size_t>(row + 1) * n + col_lo] = FromFloat<AType>(acc[1] + bias_lo);
    }
    if (hi_ok) {
      output[static_cast<size_t>(row + 1) * n + col_hi] = FromFloat<AType>(acc[3] + bias_hi);
    }
  }
}
