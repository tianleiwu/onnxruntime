...
#pragma unroll
    ...
    const uint32_t lo = static_cast<uint32_t>(e[0]) | (static_cast<uint32_t>(e[1]) << 16);
    const uint32_t hi = static_cast<uint32_t>(e[2]) | (static_cast<uint32_t>(e[3]) << 16);
    memcpy(&lo2, &lo, sizeof(lo2));
    memcpy(&hi2, &hi, sizeof(hi2));
  }
...
#pragma unroll
    ...
    if (lane == 0 && row0 + r < m) {
      ...
      y[static_cast<size_t>(row0 + r) * n + col] = FromFloat<T>(result);
    }
  }
}
...
#pragma unroll
  ...
  const int r0 = t << 1;
  const float bias_lo = (bias != nullptr && lo_ok) ? ToFloat<T>(bias[col_lo]) : 0.0f;
  const float bias_hi = (bias != nullptr && hi_ok) ? ToFloat<T>(bias[col_hi]) : 0.0f;
  if (lo_ok) {
    if (r0 < m) {
      y[static_cast<size_t>(r0) * n + col_lo] = FromFloat<T>(acc[0] * g2 + bias_lo);
    }
    if (r0 + 1 < m) {
      y[static_cast<size_t>(r0 + 1) * n + col_lo] = FromFloat<T>(acc[1] * g2 + bias_lo);
    }
  }
  if (hi_ok) {
    if (r0 < m) {
      y[static_cast<size_t>(r0) * n + col_hi] = FromFloat<T>(acc[2] * g2 + bias_hi);
    }
    if (r0 + 1 < m) {
      y[static_cast<size_t>(r0 + 1) * n + col_hi] = FromFloat<T>(acc[3] * g2 + bias_hi);
    }
  }
}
