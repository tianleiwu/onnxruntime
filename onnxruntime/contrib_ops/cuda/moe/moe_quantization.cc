...
std::optional<onnxruntime::llm::kernels::cutlass_kernels::MoeRoutePolicy> GetForcedQMoERoutePolicy() {
  ...
  if (std::strcmp(value, "gemv_ctan16") == 0) {
    return MoeRoutePolicy::GemvWhereSupported(MoeGemvConfig::kCtaN16);
  }
  if (std::strcmp(value, "gemv_threads64") == 0) {
    return MoeRoutePolicy::GemvWhereSupported(MoeGemvConfig::kThreads64);
  }
  if (std::strcmp(value, "fc1_gemv_fc2_gemm") == 0) {
    return MoeRoutePolicy::Fc1GemvFc2Gemm();
  }
  if (std::strcmp(value, "fc1_gemv_ctan16_fc2_gemm") == 0) {
    return MoeRoutePolicy::Fc1GemvFc2Gemm(MoeGemvConfig::kCtaN16);
  }
  if (std::strcmp(value, "fc1_gemv_threads64_fc2_gemm") == 0) {
    return MoeRoutePolicy::Fc1GemvFc2Gemm(MoeGemvConfig::kThreads64);
  }
  if (std::strcmp(value, "fc1_gemv_splitk2_fc2_gemm") == 0) {
    return MoeRoutePolicy::Fc1GemvFc2Gemm(MoeGemvConfig::kSplitK2);
  }
  if (std::strcmp(value, "fc1_gemv_splitk2_fc2_gemv") == 0) {
    return MoeRoutePolicy::Fc1GemvConfigFc2GemvDefault(MoeGemvConfig::kSplitK2);
  }
  if (std::strcmp(value, "fc1_gemm_fc2_gemv") == 0) {
    return MoeRoutePolicy::Fc1GemmFc2Gemv();
  }
  if (std::strcmp(value, "fc1_gemm_fc2_gemv_ctan16") == 0) {
    return MoeRoutePolicy::Fc1GemmFc2Gemv(MoeGemvConfig::kCtaN16);
  }
  if (std::strcmp(value, "fc1_gemm_fc2_gemv_threads64") == 0) {
    return MoeRoutePolicy::Fc1GemmFc2Gemv(MoeGemvConfig::kThreads64);
  }
  return std::nullopt;
}
...
std::string QMoERoutePolicyName(
  ...
  if (route_policy.fc1_route == MoeFcRoute::kGemv && route_policy.fc2_route == MoeFcRoute::kGemv) {
    return onnxruntime::MakeString("gemv_where_supported_", QMoEGemvConfigName(route_policy.fc1_gemv_config));
  }
  if (route_policy.fc1_route == MoeFcRoute::kGemv && route_policy.fc2_route == MoeFcRoute::kGroupedGemm) {
    return onnxruntime::MakeString("fc1_gemv_", QMoEGemvConfigName(route_policy.fc1_gemv_config), "_fc2_gemm");
  }
  if (route_policy.fc1_route == MoeFcRoute::kGroupedGemm && route_policy.fc2_route == MoeFcRoute::kGemv) {
    return onnxruntime::MakeString("fc1_gemm_fc2_gemv_", QMoEGemvConfigName(route_policy.fc2_gemv_config));
  }
  return "mixed_auto";
}
}  // namespace
...
Status QMoE::ComputeInternal(OpKernelContext* context) const {
  ...
  if (forced_route_policy.has_value()) {
    ...
    auto fc1_gemv_may_run_with_config = [&](onnxruntime::llm::kernels::moe_gemv::MoeGemvConfig config) {
      return is_fused_swiglu && !has_block_zero_points && parallelism_config.ep_size == 1 &&
             onnxruntime::llm::kernels::moe_gemv::is_moe_gemv_supported(
                 ...
                 static_cast<int>(expert_weight_bits_), group_size, config);
    };
    auto fc2_gemv_may_run_with_config = [&](onnxruntime::llm::kernels::moe_gemv::MoeGemvConfig config) {
      return !has_block_zero_points && parallelism_config.ep_size == 1 &&
             onnxruntime::llm::kernels::moe_gemv::is_moe_gemv_supported(
                 sm_, expanded_num_rows, moe_params.hidden_size, moe_params.inter_size,
                 static_cast<int>(expert_weight_bits_), group_size, config);
    };
    const bool fc1_gemv_may_run = fc1_gemv_may_run_with_config(
        onnxruntime::llm::kernels::moe_gemv::MoeGemvConfig::kDefault);
    const bool fc2_gemv_may_run = fc2_gemv_may_run_with_config(
        onnxruntime::llm::kernels::moe_gemv::MoeGemvConfig::kDefault);
    if (fc1_gemv_may_run || fc2_gemv_may_run) {
      ...
      if (cached_route != qmoe_route_tuning_cache_.end()) {
        ...
        using onnxruntime::llm::kernels::moe_gemv::MoeGemvConfig;
        constexpr MoeGemvConfig gemv_config_candidates[] = {
            MoeGemvConfig::kCtaN16,
            MoeGemvConfig::kThreads64,
        };
        for (MoeGemvConfig config : gemv_config_candidates) {
          const bool fc1_supported = fc1_gemv_may_run_with_config(config);
          const bool fc2_supported = fc2_gemv_may_run_with_config(config);
          if (fc1_supported && fc2_supported) {
            candidates.push_back({onnxruntime::MakeString("gemv_where_supported_", QMoEGemvConfigName(config)),
                                  MoeRoutePolicy::GemvWhereSupported(config)});
          }
          if (fc1_supported) {
            candidates.push_back({onnxruntime::MakeString("fc1_gemv_", QMoEGemvConfigName(config), "_fc2_gemm"),
                                  MoeRoutePolicy::Fc1GemvFc2Gemm(config)});
          }
          if (fc2_supported) {
            candidates.push_back({onnxruntime::MakeString("fc1_gemm_fc2_gemv_", QMoEGemvConfigName(config)),
                                  MoeRoutePolicy::Fc1GemmFc2Gemv(config)});
          }
        }
        if (is_fp16_ && fc1_gemv_may_run_with_config(MoeGemvConfig::kSplitK2)) {
          candidates.push_back({"fc1_gemv_splitk2_fc2_gemm", MoeRoutePolicy::Fc1GemvFc2Gemm(MoeGemvConfig::kSplitK2)});
          if (fc2_gemv_may_run) {
            candidates.push_back({"fc1_gemv_splitk2_fc2_gemv",
                                  MoeRoutePolicy::Fc1GemvConfigFc2GemvDefault(MoeGemvConfig::kSplitK2)});
          }
        }
        auto set_candidate_time = [&](const std::string& name, float time_ms) {
          if (name == "auto") {
            tuning_result.auto_time_ms = time_ms;
          } else if (name == "gemv_where_supported") {
            tuning_result.gemv_where_supported_time_ms = time_ms;
          } else if (name == "grouped_gemm") {
            tuning_result.grouped_gemm_time_ms = time_ms;
          } else if (name == "fc1_gemv_fc2_gemm") {
            tuning_result.fc1_gemv_fc2_gemm_time_ms = time_ms;
          } else if (name == "fc1_gemm_fc2_gemv") {
            tuning_result.fc1_gemm_fc2_gemv_time_ms = time_ms;
          }
          tuning_result.profile_log += onnxruntime::MakeString(" ", name, "_ms=", time_ms);
        };
...
#define DUMP_PACK_TENSOR(name, packed_scales, scales)
  ...
  if (input_idx == 2 && quant_type_ == "wfp4afp8" && !use_wfp4afp8_dequant_fallback_) {
    ...
    // Fused MXFP4 GEMV: lay out fc1 weights in the ColumnMajorInterleaved (kInterleave=4) layout.
    // Keep is_packed = false so the raw [E, hidden, n/2] initializer remains available for the
...
// ---------------------------------------------------------------------------
// PrePack helper: repack the raw [E, K, N/2] MXFP4 (e2m1) initializer.
//
// ``interleaved_gemv_layout == false`` (native WFP4AFP8 path): repack to the [E, N, K/2]
// row-major ColToRow layout the native CUTLASS runner consumes.
//
// ``interleaved_gemv_layout == true`` (fused FP4 GEMV path): build the interleaved
// (ColumnMajorInterleaved, kInterleave=4) layout, mirroring the INT4 MoE GEMV path. The source
// weights are [E, K, N/2] packed col-major (each byte holds two e2m1 codes along N, low nibble =
// even N) -- already the same [K, N/2] orientation the CUTLASS fpA_intB preprocessor expects as
// input, so the e2m1 codes are fed directly into ``preprocess_weights_for_mixed_gemm_cuda``
// (SM80 W4_A16) with ``apply_bias_interleave=false``. That applies only the row-permutation +
// subbyte-transpose + column-interleave (steps 1-3), producing the same positional layout as the
// INT4 interleaved MoE GEMV path. The integer bias/pair-interleave (step 4) is skipped because it
// would corrupt the floating-point e2m1 codes; the GEMV decodes them with the linear
// ``Fp4I2FConverter``, whose output order matches steps-1-3 logical order (so the
// ColumnMajorInterleaved Mapper is reused unchanged). The block scales are unaffected: with the
// interleaved kStepK=32 each thread processes exactly one 32-wide MXFP4 block per column per step,
// so the existing [E, k/32, n] fused-scale layout (TryBuildGemvFp4Scales) stays correct.
void QMoE::PrePackRepackFP4Weights(const Tensor& tensor, cudaStream_t stream, AllocatorPtr alloc,
                                   IAllocatorUniquePtr<void>& packed_buf, bool& is_packed,
                                   bool interleaved_gemv_layout) {
  auto shape = tensor.Shape();
  ...
  // Per-expert byte count: k rows x n/2 packed bytes.
  const size_t per_expert_bytes = static_cast<size_t>(k) * static_cast<size_t>(n) / 2;

  // ``preprocess_weights_for_mixed_gemm_cuda`` ping-pongs between its input and output buffers,
  // so the e2m1 source must be copied into a writable per-expert scratch before each call.
  IAllocatorUniquePtr<void> input_scratch = this->GetTransientScratchBuffer<void>(per_expert_bytes);
  int8_t* input_scratch_ptr = reinterpret_cast<int8_t*>(input_scratch.get());
  IAllocatorUniquePtr<int32_t> permutation_map = this->GetTransientScratchBuffer<int32_t>(32);

  // The QMoE int4/fp4 MoE GEMV always consumes the SM80 CUTLASS fpA_intB layout (see the note in
  // PrePackIntExpertWeights), so preprocess for SM80 regardless of the runtime device SM.
  const int packing_sm =
      onnxruntime::llm::kernels::weight_only::get_arch_for_mixed_gemm_weight_preprocess(80);
  using onnxruntime::llm::kernels::weight_only::QuantType;

  const uint8_t* src_base_gpu = static_cast<const uint8_t*>(p_src);
  for (int64_t e = 0; e < experts; ++e) {
    const uint8_t* src_e = src_base_gpu + static_cast<size_t>(e) * per_expert_bytes;
    int8_t* dst_e = dst_all + static_cast<size_t>(e) * per_expert_bytes;

    // Copy the (immutable) source expert slice into the writable scratch consumed by preprocess.
    CUDA_CALL_THROW(cudaMemcpyAsync(input_scratch_ptr, src_e, per_expert_bytes,
                                    cudaMemcpyDeviceToDevice, stream));

    // Steps 1-3 only (apply_bias_interleave=false): row-permute + subbyte-transpose +
    // column-interleave on the e2m1 codes. ``synchronize=false``: a single sync follows the loop.
    onnxruntime::llm::kernels::weight_only::preprocess_weights_for_mixed_gemm_cuda(
        stream,
        packing_sm,
        dst_e,
