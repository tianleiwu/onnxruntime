...
Status ConvertToMlasQ4Format(const uint8_t* quantized_data,
  ...
  // Transpose from N x K (weights) to K x N (B for GEMM)
  // DirectQ4Gemm expects weights to be packed in a specific layout ([K, N] logically)
  // However, the input `weights` are often provided in [N, K] format from ONNX.
  // To ensure correct computation, we must transpose the weights from [N, K] -> [K, N]
  // BEFORE packing them into the MLAS format. This resolved the "garbage output" issue.
...
Status QMoECPU<T>::Compute(OpKernelContext* context) const {
  ...
  concurrency::ThreadPool::TrySimpleParallelFor(tp, num_expert_threads, [&](std::ptrdiff_t thread_id_pd) {
    ...
    for (int64_t expert_idx : expert_batch) {
      bool fc1_bias_added_by_mlas = false;
      ...
      if (has_fc1_bias) {
        ...
        if (fc1_bias_added_by_mlas) {
          // Skip manual bias addition as it was already handled by DirectQ4Gemm
        } else {
          for (int64_t i = 0; i < num_expert_tokens; ++i) {
