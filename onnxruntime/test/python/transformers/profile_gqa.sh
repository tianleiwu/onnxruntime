...
if [ "$RUN_FP16" = true ]; then
    ...
    python parse_gqa_nsys.py gqa_fp16.sqlite --skip-first 5 --tag Fp16
fi
...
if [ "$RUN_BF16" = true ]; then
    ...
    python parse_gqa_nsys.py gqa_bf16.sqlite --skip-first 5 --tag Bf16
fi
...
if [ "$RUN_INT8" = true ]; then
    ...
    python parse_gqa_nsys.py gqa_int8.sqlite --skip-first 5 --tag Int8
fi
...
#     python parse_gqa_nsys.py gqa_int8_quant.sqlite --skip-first 5 --tag Int8Q
# fi
...
if [ "$RUN_INT4" = true ]; then
    ...
    python parse_gqa_nsys.py gqa_int4.sqlite --skip-first 5 --tag Int4
fi
