...
"""
Parse nsys SQLite output to extract GQA attention kernel timings.

Supports:
  - Flash Attention kernels (mha_fwd, flash_fwd_splitkv, etc.)
  - Memory Efficient Attention (Cutlass FMHA)
  - Rotary Embedding (RoPE)
  - KV Cache operations (ConcatKVInPlace, ConcatNewToPastKV, etc.)
  - QKV Unpack and Transpose
...
def parse_nsys_sqlite(db_path: str, kernel_patterns: list[str] | None = None, skip_first: int = 0) -> list[dict]:
    ...
    if kernel_patterns is None:
        # Comprehensive patterns for GQA-related kernels
        kernel_patterns = [
