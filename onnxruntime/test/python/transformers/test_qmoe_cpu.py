...
def quant_dequant(weights, is_4_bit_quantization: bool = True, asymmetric: bool = False):
    ...
    if torch.all(weights == 0):
        ...
        if is_4_bit_quantization:
            ...
            zero_point_storage = torch.zeros(zp_packed_size, dtype=torch.uint8, device=weights.device) if asymmetric else None
        else:
    ...
    if asymmetric:
        ...
        zero_point_int = torch.round(zero_point_float).clamp(storage_qmin, storage_qmax).to(torch.int32)  # Shape [rows, 1]
    else:
        abs_max = weights.abs().max(dim=-1, keepdim=True)[0]
        scale = abs_max / qmax_val
        scale = torch.clamp(scale, min=1e-8)
        # Symmetric zero point (storage offset)
        zero_point_int = torch.full_like(scale, (storage_qmax + 1) // 2, dtype=torch.int32)  # 8 for 4-bit, 128 for 8-bit
...
def quant_dequant_blockwise(weights, block_size, is_4_bit_quantization: bool = True, asymmetric: bool = False):
    ...
    if torch.all(weights == 0):
        ...
        if is_4_bit_quantization:
            ...
            zero_points_storage = torch.zeros((rows, zp_packed_size), dtype=torch.uint8, device=weights.device) if asymmetric else None
        else:
            quantized = torch.zeros((rows, cols), dtype=torch.uint8, device=weights.device)
            zero_points_storage = torch.zeros((rows, num_blocks), dtype=torch.uint8, device=weights.device) if asymmetric else None
        return scales, quantized, dequantized, zero_points_storage

    # Initialize output tensors
    scales = torch.zeros((rows, num_blocks), dtype=torch.float32, device=weights.device)
    zero_points_tensor = torch.zeros((rows, num_blocks), dtype=torch.int32, device=weights.device) if asymmetric else None
    dequantized = torch.zeros_like(weights)
    ...
    for row in range(rows):
        for block_idx in range(num_blocks):
            ...
            if torch.all(block_data == 0):
                ...
                quantized_block_uint8 = torch.full_like(block_data, storage_qmin if asymmetric else sym_zp_offset, dtype=torch.uint8)
                ...
                if asymmetric:
                    ...
                    quantized_block_uint8 = torch.round(quantized_block_float).clamp(storage_qmin, storage_qmax).to(torch.uint8)

                    dequantized[row, start_col:end_col] = (quantized_block_uint8.float() - zero_point_int.float()) * scale.float()
                    ...
                    quantized_block_signed = torch.round(block_data.double() / scale.double()).clamp(qmin_val, qmax_val).to(torch.int32)
            ...
            if is_4_bit_quantization:
                for i in range(0, end_col - start_col, 2):
                    ...
                    val2 = int(quantized_block_uint8[i + 1]) if i + 1 < len(quantized_block_uint8) else storage_qmin if asymmetric else sym_zp_offset
...
class TestPhiQMoECPU(unittest.TestCase):
    ...
    def test_phi3_qmoe_asymmetric_parity_cpu(self, batch_size, sequence_length, quant_bits):
        ...
        test_config = f"batch_size={batch_size}, sequence_length={sequence_length}, quant_bits={quant_bits}, seed={unique_seed}"
