...
class GroupQueryAttentionConfig(AttentionConfig):
    ...
    def random_inputs(self):
        ...
        if self.k_quant_type != "NONE":
            # Use float16 for quantization computation
            dtype = torch.float16 if self.dtype == torch.bfloat16 else self.dtype
            ...
            # Convert scales to the appropriate dtype
            if k_scale is not None:
                k_scale = k_scale.to(dtype)
                feeds["k_scale"] = k_scale
            if v_scale is not None:
                v_scale = v_scale.to(dtype)
...
def create_group_query_attention_onnx_model(config: GroupQueryAttentionConfig):
    ...
    if config.k_quant_type != "NONE":
        ...
        graph_input.append(helper.make_tensor_value_info("k_scale", float_type, k_scale_shape))
    ...
    if config.v_quant_type != "NONE":
        ...
        graph_input.append(helper.make_tensor_value_info("v_scale", float_type, v_scale_shape))
...
# Do not run too many tests. Change it to True to run all combinations.
comprehensive_mode = False


@unittest.skipIf(os.getenv("PIPELINE_MODE", "1") == "1", "Skipping tests in pipeline mode.")
