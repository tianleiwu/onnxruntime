...
using ReduceComputeStreamT = Stream*;
#else
using ReduceComputeStreamT = cudaStream_t;
#endif

template <typename T>
inline IAllocatorUniquePtr<T> AllocateScratchBuffer(
    const AllocatorPtr& gpu_allocator, const CudaKernel* kernel, size_t count, ReduceComputeStreamT stream) {
  if (count == 0) return nullptr;
#ifndef BUILD_CUDA_EP_AS_PLUGIN
  ORT_UNUSED_PARAMETER(kernel);
  return IAllocator::MakeUniquePtr<T>(gpu_allocator, count, false, stream);
#else
  ORT_UNUSED_PARAMETER(gpu_allocator);
  return kernel->GetScratchBuffer<T>(count, stream);
#endif
}
...
Status ReduceComputeCore(const AllocatorPtr& gpu_allocator, const CudaKernel* kernel, const Tensor& input, PrepareReduceMetadata& prepare_reduce_metadata,
                         ...
                         ReduceComputeStreamT ort_stream,
                         const TensorShape* input_shape_override) {
  typedef typename ToCudaType<T>::MappedType CudaT;
...
#else
  cudaStream_t stream = ort_stream;
#endif
  ...
  if (fast_reduction) {
    ...
    if (applicable_matrix_reduction != ApplicableMatrixReduction::None) {
      ...
      if (calculate_sqt) {
        input_data_buffer = AllocateScratchBuffer<T>(gpu_allocator, kernel, input_count, ort_stream);
        input_data = reinterpret_cast<CudaT*>(input_data_buffer.get());
        fast_divmod tmp_div;
      ...
      switch (applicable_matrix_reduction) {
        ...
        case ApplicableMatrixReduction::Columns: {
          ...
          auto buffer = buffer_size_bytes == 0
                            ? nullptr
                            : AllocateScratchBuffer<void>(gpu_allocator, kernel, buffer_size_bytes, ort_stream);
  ...
  if ((ReduceTensorIndices == CUDNN_REDUCE_TENSOR_FLATTENED_INDICES && std::is_same<T, MLFloat16>::value) ||
    ...
    // ArgMax/ArgMin with FP16 are not supported by cudnn, so convert input to fp32 then call cudnn
    temp_X = AllocateScratchBuffer<float>(gpu_allocator, kernel, input_count, ort_stream);
  ...
  ORT_RETURN_IF_ERROR(output_tensor.Set(output_dims_cudnn, cudnn_type_X));
  size_t workspace_bytes = 0;
  cudnnHandle_t cudnn_handle = CudaKernel::GetCudnnHandle(ort_stream);
  ...
  auto workspace_cuda = workspace_bytes == 0
                            ? nullptr
                            : AllocateScratchBuffer<CudaT>(gpu_allocator, kernel, workspace_bytes, ort_stream);
  ...
  auto indices_cuda = indices_bytes == 0
                          ? nullptr
                          : AllocateScratchBuffer<uint32_t>(gpu_allocator, kernel, indices_bytes, ort_stream);
  ...
  if (ReduceTensorIndices == CUDNN_REDUCE_TENSOR_NO_INDICES) {
    ...
    if (calculate_sqt) {
      ...
      auto exp_result_buffer = AllocateScratchBuffer<T>(gpu_allocator, kernel, input_count, ort_stream);
      auto exp_result = exp_result_buffer.get();
      auto log_sum_result_buffer = output_count == 0
                                       ? nullptr
                                       : AllocateScratchBuffer<T>(gpu_allocator, kernel, output_count, ort_stream);
      auto log_sum_result = log_sum_result_buffer.get();
      BinaryElementwisePreparation prepare;
      ...
      if (input_count == output_count) {
        ...
        if (temp_X) {
          auto temp_output = output_count == 0
                                 ? nullptr
                                 : AllocateScratchBuffer<float>(gpu_allocator, kernel, output_count, ort_stream);
          CUDNN_RETURN_IF_ERROR(cudnnReduceTensor(
              cudnn_handle, reduce_desc, indices_cuda.get(), indices_bytes,
              workspace_cuda.get(), workspace_bytes,
              &one, input_tensor, temp_X.get(),
    ...
    if (input_count == output_count) {
      ...
      if (temp_X) {
        ...
        auto temp_output = output_count == 0
                               ? nullptr
                               : AllocateScratchBuffer<CudaT>(gpu_allocator, kernel, output_count, ort_stream);
        CUDNN_RETURN_IF_ERROR(cudnnReduceTensor(
            cudnn_handle, reduce_desc, indices_cuda.get(), indices_bytes,
            workspace_cuda.get(), workspace_bytes,
            &one, input_tensor, reinterpret_cast<const CudaT*>(input.Data<T>()),
...
#ifndef BUILD_CUDA_EP_AS_PLUGIN
  AllocatorPtr allocator = Info().GetAllocator(OrtMemType::OrtMemTypeDefault);
