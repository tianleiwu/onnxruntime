...
CudaEpFactory::CudaEpFactory(const OrtApi& ort_api, const OrtEpApi& ep_api,
  ...
  cpu_allocator_ = CPUAllocator::DefaultInstance();
  device_allocator_ = std::make_shared<CudaDeviceIAllocator>(device_id_);
  pinned_allocator_ = std::make_shared<CudaPinnedIAllocator>(device_id_);
}
...
OrtStatus* ORT_API_CALL CudaEpFactory::CreateEpImpl(
  ...
  *ep = nullptr;
  ...
  config.device_id = factory->device_id_;

  const OrtLogger& ep_logger = logger ? *logger : factory->default_logger_;
  auto provider = std::make_unique<CUDAExecutionProvider>(
      config.device_id, config.use_tf32, config.cudnn_conv_algo,
      config.cudnn_conv1d_pad_to_nc1d, config.cudnn_conv_use_max_workspace,
      config.enable_skip_layer_norm_strict_mode, config.prefer_nhwc);
  factory->active_provider_ = provider.get();
  auto actual_ep = std::make_unique<CudaEp>(std::move(provider), *factory, config, ep_logger,
...
void ORT_API_CALL CudaEpFactory::ReleaseEpImpl(OrtEpFactory* this_ptr, OrtEp* ep) noexcept {
  auto* cuda_ep = static_cast<CudaEp*>(ep);
  auto* provider = static_cast<CUDAExecutionProvider*>(cuda_ep->EpImpl());
  auto* factory = static_cast<CudaEpFactory*>(this_ptr);
  if (factory->active_provider_ == provider) {
    factory->active_provider_ = nullptr;
  }
  delete cuda_ep;
}
...
OrtStatus* ORT_API_CALL CudaEpFactory::CreateAllocatorImpl(
  ...
  *allocator = nullptr;
  ...
  if (is_default) {
    *allocator = new onnxruntime::ep::adapter::Allocator(memory_info, factory.device_allocator_);
    return nullptr;
  }

  if (is_pinned) {
    *allocator = new onnxruntime::ep::adapter::Allocator(memory_info, factory.pinned_allocator_);
    return nullptr;
  }
...
void ORT_API_CALL CudaEpFactory::ReleaseAllocatorImpl(
  ...
  delete static_cast<onnxruntime::ep::adapter::Allocator*>(allocator);
}
...
OrtStatus* ORT_API_CALL CudaEpFactory::CreateSyncStreamForDeviceImpl(
  ...
  auto* factory = static_cast<CudaEpFactory*>(this_ptr);
  if (factory->active_provider_ == nullptr) {
    return factory->ort_api_.CreateStatus(
        ORT_FAIL,
        "CUDA EP sync stream creation requires an active provider instance.");
  }

  auto cuda_stream = std::make_unique<CudaSyncStream>(
      *factory, *factory->active_provider_, factory->device_id_, nullptr);
