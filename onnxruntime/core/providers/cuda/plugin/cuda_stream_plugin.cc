...
CudaSyncStream::CudaSyncStream(CudaEpFactory& factory, CUDAExecutionProvider& provider, int device_id,
                               const OrtEp* /*ep*/)
    : OrtSyncStreamImpl{},
      factory_(factory),
      provider_(provider),
...
CudaSyncStream::~CudaSyncStream() {
  ...
  if (cuda_stream_) provider_.UnregisterStream(cuda_stream_);
