...
CudaEp::CudaEp(std::unique_ptr<CUDAExecutionProvider> provider,
               CudaEpFactory& factory,
               const Config& config,
               const OrtLogger& logger,
               AllocatorPtr cpu_allocator,
               AllocatorPtr device_allocator)
    : onnxruntime::ep::adapter::Ep{std::move(provider), cpu_allocator, device_allocator},
      factory_(factory),
...
OrtStatus* ORT_API_CALL CudaEp::OnRunStartImpl(
  ...
  auto* provider = static_cast<CUDAExecutionProvider*>(ep->EpImpl());
  CudaSyncStream* compute_stream = provider ? provider->GetComputeSyncStream() : nullptr;
