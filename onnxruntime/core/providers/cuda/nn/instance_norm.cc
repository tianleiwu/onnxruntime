...
Status InstanceNorm<T>::ComputeInternal(OpKernelContext* p_op_kernel_context) const {
  ...
  if (N == 1) {
    ...
    auto mean = GetScratchBuffer<CudaT>(stats_count, GetScratchStream(p_op_kernel_context));
    CUDA_RETURN_IF_ERROR(cudaMemsetAsync(mean.get(), 0, stats_byte_count, Stream(p_op_kernel_context)));
    auto variance = GetScratchBuffer<CudaT>(stats_count, GetScratchStream(p_op_kernel_context));
    ...
    auto unused_scale = GetScratchBuffer<CudaT>(stats_count, GetScratchStream(p_op_kernel_context));
    CUDA_RETURN_IF_ERROR(cudaMemsetAsync(unused_scale.get(), 0, stats_byte_count, Stream(p_op_kernel_context)));
    auto unused_bias = GetScratchBuffer<CudaT>(stats_count, GetScratchStream(p_op_kernel_context));
...
Status InstanceNorm<MLFloat16>::ComputeInternal(OpKernelContext* p_op_kernel_context) const {
  ...
  if (N == 1) {
    ...
    auto scale_data_fp32 = GetScratchBuffer<float>(C, GetScratchStream(p_op_kernel_context));
    Impl_Cast<CudaT, float>(Stream(p_op_kernel_context), scale_data, scale_data_fp32.get(), C);

    auto bias_data_fp32 = GetScratchBuffer<float>(C, GetScratchStream(p_op_kernel_context));
    ...
    // Mean & Variance are inputs & outputs and must be initialized to zero to work properly
    auto mean = GetScratchBuffer<float>(stats_count, GetScratchStream(p_op_kernel_context));
    CUDA_RETURN_IF_ERROR(cudaMemsetAsync(mean.get(), 0, stats_byte_count, Stream(p_op_kernel_context)));
    auto variance = GetScratchBuffer<float>(stats_count, GetScratchStream(p_op_kernel_context));
    CUDA_RETURN_IF_ERROR(cudaMemsetAsync(variance.get(), 0, stats_byte_count, Stream(p_op_kernel_context)));

    // We must set the scale & bias inputs to zero as they are inputs to the calculation
    auto unused_scale = GetScratchBuffer<float>(stats_count, GetScratchStream(p_op_kernel_context));
    CUDA_RETURN_IF_ERROR(cudaMemsetAsync(unused_scale.get(), 0, stats_byte_count, Stream(p_op_kernel_context)));
    auto unused_bias = GetScratchBuffer<float>(stats_count, GetScratchStream(p_op_kernel_context));
    CUDA_RETURN_IF_ERROR(cudaMemsetAsync(unused_bias.get(), 0, stats_byte_count, Stream(p_op_kernel_context)));
