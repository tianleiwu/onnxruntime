...
Status Conv<T, NHWC>::UpdateState(OpKernelContext* context, bool bias_expected) const {
  ...
  if (input_dims_changed || w_dims_changed) {
    ...
    if (post_slicing_required) {
      ...
      s_.memory_for_cudnn_conv_results = GetScratchBuffer<void>(TensorShape(y_dims_with_adjusted_pads).Size() * s_.element_size, Stream(context));
    ...
    if (s_.post_slicing_required) {
      s_.memory_for_cudnn_conv_results = GetScratchBuffer<void>(TensorShape(s_.y_dims_with_adjusted_pads).Size() * s_.element_size, Stream(context));
      s_.y_data = reinterpret_cast<CudaT*>(s_.memory_for_cudnn_conv_results.get());
    } else {
      s_.y_data = reinterpret_cast<CudaT*>(s_.Y->MutableData<T>());
    }
...
Status Conv<T, NHWC>::ComputeInternal(OpKernelContext* context) const {
  ...
  IAllocatorUniquePtr<void> workspace = GetWorkSpace(Stream(context));
