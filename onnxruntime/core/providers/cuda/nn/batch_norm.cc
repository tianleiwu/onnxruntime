...
Status BatchNorm<T, NHWC>::ComputeInternal(OpKernelContext* p_op_kernel_context) const {
  ...
  if (X->IsDataType<MLFloat16>()) {
    ...
    auto f_scale = GetScratchBuffer<float>(C, GetScratchStream(p_op_kernel_context));
    auto f_B = GetScratchBuffer<float>(C, GetScratchStream(p_op_kernel_context));
    auto f_mean = GetScratchBuffer<float>(C, GetScratchStream(p_op_kernel_context));
    auto f_var = GetScratchBuffer<float>(C, GetScratchStream(p_op_kernel_context));
