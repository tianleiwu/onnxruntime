...
inline Status CheckROIAlignValidInput(const Tensor* X_ptr, const Tensor* rois_ptr, const Tensor* batch_indices_ptr) {
  constexpr int64_t kExpectedNumRoiDims = 2;
  constexpr int64_t kExpectedSecondRoiDim = 4;
  ...
  if (rois_dims.NumDimensions() != kExpectedNumRoiDims) {
    return Status(common::ONNXRUNTIME, common::INVALID_ARGUMENT,
                  "Number of dimensions for rois should be exactly " + std::to_string(kExpectedNumRoiDims));
  }
  if (rois_dims[1] != kExpectedSecondRoiDim) {
    return Status(common::ONNXRUNTIME, common::INVALID_ARGUMENT,
                  "Second dimension for rois should be exactly " + std::to_string(kExpectedSecondRoiDim));
  }
  ...
  if (batch_indices_ptr->Location().device.Type() == OrtDevice::CPU) {
    ...
    if (batch_indices_ptr->IsDataType<int64_t>()) {
      ORT_RETURN_IF_ERROR(check_bounds(batch_indices_ptr->Data<int64_t>()));
    } else if (batch_indices_ptr->IsDataType<int32_t>()) {
      ORT_RETURN_IF_ERROR(check_bounds(batch_indices_ptr->Data<int32_t>()));
    } else {
      return Status(common::ONNXRUNTIME, common::INVALID_ARGUMENT,
 ...
 public:
  ...
  explicit RoiAlignBase(const TKernelInfo& info) {
    ...
    if (info.template GetAttr<std::string>("mode", &mode).IsOK()) {
      std::transform(mode.begin(), mode.end(), mode.begin(), [](char c) { return static_cast<char>(::tolower(c)); });
      if (mode == "avg") {
