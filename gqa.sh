...
if [ "$RUN_BUILD" = true ]; then
    ...
    sh build.sh --config $BUILD_TYPE  --build_dir build/cuda --parallel  --use_cuda \
            ...
            --cmake_extra_defines onnxruntime_QUICK_BUILD=OFF \
...
if [ "$RUN_INSTALL" = true ]; then
    ...
    pip install build/cuda/$BUILD_TYPE/dist/onnxruntime_gpu-1.25.0-cp312-cp312-linux_x86_64.whl --force-reinstall --no-cache-dir
fi
